...
import {cropDetectedDocument, Point as PolygonPoint} from '../utils/polygonCropper';
import {quickTest} from '../utils/cropDiagnostic';
import '../utils/coverModeTest'; // This will run the cover mode test on import
  ...
  const captureDocument = async (auto = false) => {
    try {
      ...
      if (cameraRef.current && isCameraReady && device) {
        ...
        // 🔍 DIAGNOSTIC: Test the fixed coordinate mapping
        ...
        // Check if we have detected corners for polygon-based cropping
        // if (capturedCorners && capturedCorners.length === 4) {
        //   console.log('[Capture] Using polygon-based cropping with detected corners:', capturedCorners);
          
        //   // Convert frame-space corners to photo-space coordinates
        //   const photoSpaceCorners: PolygonPoint[] = capturedCorners.map(corner => ({
        //     x: corner.x * (photoSize.width / 1920), // Assuming frame size of 1920x1080
        //     y: corner.y * (photoSize.height / 1080)
        //   }));
          
        //   console.log('[Capture] Photo-space polygon corners:', photoSpaceCorners);
          
        //   // Use polygon cropper for precise document cropping
        //   const polygonCropResult = await cropDetectedDocument(imageUri, photoSpaceCorners, {
        //     padding: 20,
        //     quality: 0.9
        //   });
          
        //   if (polygonCropResult.success) {
        //     console.log('[Capture] Polygon cropping successful:', polygonCropResult.croppedUri);
            
        //     // Navigate to document review screen for polygon cropping
        //     console.log('[Capture] Would navigate to DocumentReview with polygon data');
        //     // Temporarily disabled navigation to test imports
        //     // navigation.navigate('DocumentReview', {
        //     //   step: currentStep,
        //     //   originalImageUri: imageUri,
        //     //   detectedCorners: photoSpaceCorners,
        //     //   bounds: cropBounds,
        //     // });
        //     // return;
        //   } else {
        //     console.warn('[Capture] Polygon cropping failed, falling back to frame cropping:', polygonCropResult.error);
        //   }
        // }

        console.log('[Capture] Falling back to frame cropping', cropBounds);
        // Use the full frame bounds for cropping (the border is just visual)
        const cropAreaBounds = cropBounds;
        console.log('[Capture] Using frame bounds for cropping:', cropAreaBounds);
        
        // Fallback: Use the enhanced FrameCropper for blue dotted frame
        ...
        const cropResult = await FrameCropper.cropToFrame(
          imageUri,
          cropAreaBounds as FrameBounds,
          photoSize, // 🔥 FIX: Use actual photo dimensions, not frame dimensions!
          {
            addPadding: true,
            paddingPercent: 0.02, // Small padding for frame-based crop
            containerSize: overlayLayout || undefined, // Pass the camera container dimensions
          }
        );
        console.log('[Capture] Falling back to frame cropping', cropResult.croppedUri);
